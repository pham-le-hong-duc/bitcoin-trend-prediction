...
class Consumer:
    ...
    def __init__(self,
        ...
        Args:
            ...
            file_prefix: Optional filename prefix (e.g., 'RS' -> RS_2026-05.parquet)
        ...
        self.minio_writer = MinIOWriter(bucket=minio_bucket)
    ...
    def _align_schema(self, df):
        """Align a DataFrame to the configured schema order before concatenation."""
        if df is None or not self.column_names:
            return df
    ...
    def _group_by_period(self, records):
        ...
        for record in records:
            ...
            if isinstance(ts, str):
                ...
                # Integer timestamp in seconds or milliseconds
    ...
    def _write_to_minio(self, records):
        ...
        for period_str, period_records in grouped.items():
            try:
                ...
                filename = (
                    f"{self.file_prefix}_{period_str}.parquet"
                    if self.file_prefix
                    else f"{period_str}.parquet"
                )
                object_path = f"{self.data_type}/{filename}"
                
                # Check if file exists
                existing_df = self.minio_writer.read_parquet(object_path)
                
                if existing_df is not None:
                    existing_df = self._align_schema(existing_df)
