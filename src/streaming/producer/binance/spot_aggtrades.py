"""
Binance Spot Aggregate Trades Producer

Streams aggregate trade data from Binance Spot WebSocket and produces to Redpanda.
Schema matches spot-aggTrades.csv format.
"""
import asyncio
import os
import logging
...
class BinanceSpotAggTradesProducer:
  """
  Producer for Binance Spot Aggregate Trades.
  
  Schema (spot-aggTrades.csv):
  ...
  def __init__(self, 
    ...
    self.client = Spot(config_ws_streams=configuration_ws_streams)
  ...
  def transform_message(self, data):
    ...
    Binance SDK returns AggTradeResponse object with attributes:
    - e: event type
    - E: event time
    - s: symbol
...
async def main():
  ...
  topic = os.getenv("TOPIC", "binance-spot-aggTrades")
  
  # Create and start producer
  producer = BinanceSpotAggTradesProducer(
    symbol=symbol,
    bootstrap_servers=bootstrap_servers,
    topic=topic
  )
