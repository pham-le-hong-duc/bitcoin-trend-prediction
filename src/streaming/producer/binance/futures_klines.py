"""
Binance Futures Klines Producer
Streams BTCUSDT 1m futures kline data from Binance Futures to Redpanda
"""
import os
import asyncio
import logging
...
TOPIC = os.getenv('TOPIC', 'binance-futures-klines')
...
def _sync_fetch_futures_klines():
...
async def fetch_futures_klines():
  """Fetch the most recent futures klines in a thread to avoid blocking."""
  return await asyncio.to_thread(_sync_fetch_futures_klines)
...
async def poll_until_new_data(expected_open_time):
  ...
  while True:
    ...
    try:
      klines = await fetch_futures_klines()
