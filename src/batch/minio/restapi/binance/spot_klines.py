"""
Binance Spot Klines REST API backfill.
...
class BinanceSpotKlines(RestAPI):
    """Backfill Binance Spot klines data using REST API."""

    def __init__(self, symbol="BTCUSDT", interval="1m", data_type="spot/daily/klines/BTCUSDT/1m"):
        super().__init__(
            symbol=symbol,
            data_type=data_type,
            client_type="spot",
    ...
    def get_api_data(self, start_date, end_date):
        """Fetch spot klines from Binance API with pagination."""
        start_ms = int(start_date.timestamp() * 1000)
        end_ms = int(end_date.timestamp() * 1000)
        current_start = start_ms

        interval_enum = KlinesIntervalEnum[f"INTERVAL_{self.interval}"].value
...
def main():
    """Run Binance Spot Klines backfill."""
    backfill = BinanceSpotKlines(symbol="BTCUSDT")
    backfill.run()
