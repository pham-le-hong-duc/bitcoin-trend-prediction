"""
Binance Futures Mark Price Klines REST API backfill.
...
class BinanceFuturesMarkPriceKlines(RestAPI):
    """Backfill Binance Futures mark price klines data using REST API."""
    
    def __init__(self, symbol="BTCUSDT", interval="1m", data_type="futures/um/daily/markPriceKlines/BTCUSDT/1m"):
        """
        Initialize Binance Futures Mark Price Klines backfill.
    ...
    def get_api_data(self, start_date, end_date):
        """Fetch mark price klines from Binance API with pagination."""
        start_ms = int(start_date.timestamp() * 1000)
        end_ms = int(end_date.timestamp() * 1000)
        current_start = start_ms
        
        interval_enum = MarkPriceKlineCandlestickDataIntervalEnum[f"INTERVAL_{self.interval}"].value
...
def main():
    """Run Binance Futures Mark Price Klines backfill."""
    backfill = BinanceFuturesMarkPriceKlines(symbol="BTCUSDT")
    backfill.run()
