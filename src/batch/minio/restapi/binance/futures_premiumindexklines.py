"""
Binance Futures Premium Index Klines REST API backfill.
...
class BinanceFuturesPremiumIndexKlines(RestAPI):
    """Backfill Binance Futures premium index klines data using REST API."""
    
    def __init__(self, symbol="BTCUSDT", interval="1m", data_type="futures/um/daily/premiumIndexKlines/BTCUSDT/1m"):
        """
        Initialize Binance Futures Premium Index Klines backfill.
    ...
    def get_api_data(self, start_date, end_date):
        """Fetch premium index klines from Binance API with pagination."""
        start_ms = int(start_date.timestamp() * 1000)
        end_ms = int(end_date.timestamp() * 1000)
        current_start = start_ms
        
        interval_enum = PremiumIndexKlineDataIntervalEnum[f"INTERVAL_{self.interval}"].value
        ...
        while current_start < end_ms:
            ...
            for retry_count in range(max_retries):
                try:
                    response = self.client.rest_api.premium_index_kline_data(
                        symbol=self.symbol,
...
def main():
    """Run Binance Futures Premium Index Klines backfill."""
    backfill = BinanceFuturesPremiumIndexKlines(symbol="BTCUSDT")
    backfill.run()
