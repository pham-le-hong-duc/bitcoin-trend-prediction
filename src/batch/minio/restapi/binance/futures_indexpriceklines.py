"""
Binance Futures Index Price Klines REST API backfill.
...
class BinanceFuturesIndexPriceKlines(RestAPI):
    """Backfill Binance Futures index price klines data using REST API."""
    
    def __init__(self, symbol="BTCUSDT", interval="1m", data_type="futures/um/daily/indexPriceKlines/BTCUSDT/1m"):
        """
        Initialize Binance Futures Index Price Klines backfill.
    ...
    def get_api_data(self, start_date, end_date):
        """Fetch index price klines from Binance API with pagination."""
        start_ms = int(start_date.timestamp() * 1000)
        end_ms = int(end_date.timestamp() * 1000)
        current_start = start_ms
        
        interval_enum = IndexPriceKlineCandlestickDataIntervalEnum[f"INTERVAL_{self.interval}"].value
        ...
        while current_start < end_ms:
            ...
            for retry_count in range(max_retries):
                try:
                    response = self.client.rest_api.index_price_kline_candlestick_data(
                        pair=self.symbol,
    ...
    def transform_data(self, api_response):
        ...
        return pl.DataFrame(transformed_records)  # Keep timestamps as integer


def main():
    """Run Binance Futures Index Price Klines backfill."""
    backfill = BinanceFuturesIndexPriceKlines(symbol="BTCUSDT")
    backfill.run()
