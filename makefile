...
# 2. producer-up        -> Start streaming producer (docker/docker-compose.streaming.producer.yml)
# 3. consumer.minio-up  -> Start MinIO consumer (docker/docker-compose.streaming.consumer.minio.yml)
# 4. batch_minio        -> Trigger batch_minio DAG (dags/batch_minio.py)

.PHONY: infra-up producer-up consumer.minio-up batch_minio up down start-all build
...
# Combined operations
up: infra-up producer-up consumer.minio-up batch_minio

down:
	@docker-compose -f docker/docker-compose.infrastructure.yml -f docker/docker-compose.streaming.producer.yml -f docker/docker-compose.streaming.consumer.minio.yml down
